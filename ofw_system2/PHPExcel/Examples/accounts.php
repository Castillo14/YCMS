...
$objPHPExcel->getActiveSheet(0)->setCellValue('B1', 'Account Name');
...
$objPHPExcel->getActiveSheet(0)->setCellValue('F1', 'Job');

foreach(range('A', 'F') as $cols){
...
$row_count = 3;
$i=0;
$query = mysqli_query($con,"SELECT * FROM tbl_accounts WHERE job = 'Administrator' OR job = 'Staff' AND removed = 'No'");
...
$objPHPExcel->getActiveSheet(0)->setCellValue('F' .$row_count,$job);
