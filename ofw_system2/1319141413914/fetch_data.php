<?php
	...
	$connect = new PDO("mysql:host=localhost;dbname=u577886912_ofw3", "u577886912_ofw3", "ofw3ofw3");
	...
	if($_POST['query'] != ''){
		...
		$query = "SELECT * FROM tbl_applicants WHERE application_status = '1' AND removed = 'No' AND applicant_id IN (".$search_text.") ";
	...
	if($total_row > 0){
		...
		foreach($result as $row){
			...
			$get_emp = "SELECT * FROM tbl_employers WHERE employer_id = '$employer_id' AND jo_id = '$jo_id'";
            ...
            $get_jo = "SELECT * FROM tbl_job_order WHERE jo_id = '$jo_id'";
			
			$run_jo = $connect->prepare($get_jo);

			$run_jo->execute();

            $row_jo = $run_jo->fetchAll();

            $total_jo = $run_jo->rowCount();

            if($total_jo > 0){

            	foreach($row_jo as $row_joo){

            		$joname = $row_joo['jo_name'];
            ...
            $get_agent = "SELECT * FROM tbl_agents WHERE agent_id = '$agent_id'";
			
			$run_agent = $connect->prepare($get_agent);

			$run_agent->execute();

            $row_agent = $run_agent->fetchAll();

            $total_agent = $run_agent->rowCount();

            if($total_agent > 0){

            	foreach($row_agent as $row_agentt){

            		$fnamee = $row_agentt['first_name'];
            		$mnamee = $row_agentt['middle_name'];
            		$lnamee = $row_agentt['last_name'];

		            $full_nn = ucfirst($lnamee) . ", " . ucfirst($fnamee) . " " . ucfirst($mnamee);
				...
				<form class='form-horizontal' action='add_processing_applcnt.php' method='post'><!-- form-horizontal Starts -->
	                ...
	                <div class='col-md-6'><!-- col-md-6 Starts -->
	                	<div class='form-group'>
	                        ...
	                        <div class='col-sm-8'>
	                        <input type='text' class='form-control' name='jo' value='$joname' disabled>
	                        </div>  
	                    </div>
	                    <div class='form-group'>
	                        <label class='control-label col-sm-4'>
	                    ...
	                    <div class='form-group'>
	                        ...
	                        <div class='col-sm-8'>
	                        <input type='text' class='form-control' name='agent' value='$full_nn' disabled>
	                        </div>  
	                    </div>
	                </div><!-- col-md-6 Ends -->
	                <br>
